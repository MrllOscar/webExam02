...
<!-- fieldset、legend做框，標題用span寫活透過點擊改變 -->
<div>目前位置:首頁 > 分類網誌 <span id="word"> >健康新知</span></div>
<fieldset>
  <legend>分類網誌</legend>
<div class="tagbox">
  <div id="tag1" class="tag">健康新知</div>
  <div id="tag2" class="tag">菸害防制</div>
  <div id="tag3" class="tag">癌症防治</div>
  <div id="tag4" class="tag">慢性病防治</div>
</div>
</fieldset>
<!-- 複製一份fieldset，id多t讓home頁的js找到這層、底下原本的tag在多一個t(利用ctrl+d快速選取後增加)，
class多一個ttag判斷點擊，並且能改變最上方目前位置的文字為'>'+$(this).val() -->
<!-- fieldset屁股包到文末，改legend -->
<!-- foreach只找type跟標題，內文一樣直接寫死 -->
<fieldset>
  <legend>文章列表</legend>
  <div id="ttag1" class="block ttag">
  <?php $row=$Text->all(['type'=>1]);
      foreach($row as $v){
        echo $v['title'].'<br>';
      }
  ?></div>
<div id="ttag2" class="block ttag">
<?php $row=$Text->all(['type'=>2]);
      foreach($row as $v){
        echo $v['title'].'<br>';
      }
  ?>
</div>
<div id="ttag3" class="block ttag">
<?php $row=$Text->all(['type'=>3]);
      foreach($row as $v){
        echo $v['title'].'<br>';
      }
  ?>
</div>
<div id="ttag4" class="block ttag">
<?php $row=$Text->all(['type'=>4]);
      foreach($row as $v){
        echo $v['title'].'<br>';
      }
  ?>
