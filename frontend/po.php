<!-- 從0開始，直接複製文檔貼上id用tag跟ttag來調整js顯示方式，注意文章還有h2標題一次 -->
<!-- css最少的辨識方式 -->
<!-- 最下面js直接透過tag被點擊，找到id，將block隱藏id=#t+被點的id顯示 -->
<style>
  .block:not(#ttag1){
    display: none;
  }
</style>
<div>目前位置：首頁 > 分類網誌 <span id="word"> > 健康新知</span></div>
<div class="tagbox">
  <button id="tag1" class="tag">健康新知</button>
  <button id="tag2" class="tag">菸害防制</button>
  <button id="tag3" class="tag">癌症防治</button>
  <button id="tag4" class="tag">慢性病防治</button>
</div>
<div class="tagbox">
  <span id="ttag1" class="ttag block">缺乏運動已成為影響全球死亡率的第四大危險因子-國人無規律運動之比率高達72.2%(1)<br>缺乏運動已成為影響全球死亡率的第四大危險因子-國人無規律運動之比率高達72.2%(2)</span>
  <span id="ttag2" class="ttag block">菸害防制法規(1)<br>菸害防制法規(2)</span>
  <span id="ttag3" class="ttag block">降低罹癌風險 建構健康生活型態(1)<br>降低罹癌風險 建構健康生活型態(2)</span>
  <span id="ttag4" class="ttag block">長期憋尿 泌尿系統問題多(1)<br>長期憋尿 泌尿系統問題多(2) </span>
</div>

<pre id="tttag1" class="block">
資料來源： 行政院衛生署國民健康局 
發佈日期： 2012 / 10 / 07
...
<script>
    ...
    $('.tag').on('click',function(){
      let showwho=$(this).attr('id');
      let showwhat=$(this).html();
      $('.block').hide();
      $('#t'+showwho).show();
      $("#word").html('> '+showwhat);
    })
